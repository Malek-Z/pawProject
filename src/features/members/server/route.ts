...
const app = new Hono()
    ...
    .get("/" , sessionMiddleware , zValidator("query", z.object({ workspaceId: z.string() })) , async (c) => {
        ...
        return c.json({ data: {...members, documents: populateMembers} })
    ...
    .delete("/:memberId" , sessionMiddleware, async (c) => {
        ...
        if ( member.id !== memberToDelete.$id && member.role !== MemberRole.ADMIN ) c.json({ error: "UnAuthorized" }, 401);
    ...
    .patch("/:memberId" , sessionMiddleware, zValidator("json" , z.object({ role: z.nativeEnum(MemberRole) })) , async (c) => {
        ...
        if ( member.role !== MemberRole.ADMIN ) c.json({ error: "UnAuthorized" }, 401);
